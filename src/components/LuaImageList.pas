...
Uses Lua, LuaController, TypInfo, LuaVmt, ImgList, Controls;
...
function VCLua_ImageList_AddMultipleResolutions2(L: Plua_State): Integer; cdecl;
var
	lImageList:TLuaImageList;
...
begin
	CheckArg(L, 2);
	lImageList := TLuaImageList(GetLuaObject(L, 1));
	TTrait<TRasterImage>.luaL_checkArray(L, 2, @Images);
	try
		ret := lImageList.AddMultipleResolutions(Images);
		Result := 1;
	except
...
begin
	...
	Result := 1;
		...
		on E: Exception do
			CallError(L, 'VCL', 'ImageList', E.ClassName, E.Message);
	end;
end;
...
begin
	...
	TLuaMethodInfo.Create(CustomImageListFuncs, 'AddMultipleResolutions2', @VCLua_ImageList_AddMultipleResolutions2);
