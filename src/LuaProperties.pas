...
begin
  ...
  if PInfo <> nil then begin
    ...
    case lua_type(L,1) of
      ...
      LUA_TTABLE: begin
          // VCLua 0.7.1
          // non published properties, so diy
          cName := TObject(Comp).ClassName;
          if lowercase(lua_tostring(L,2)) = 'classname' then begin
             lua_pushstring(L,pchar(cName));
          end else
          ...
          if lowercase(lua_tostring(L,2)) = 'canvas' then begin
             // eg. form
             if (comp.InheritsFrom(TCustomControl)) then
                lua_pushobject(L, TCustomControl(Comp).Canvas, -1)
             else
             // eg. shape
             if (comp.InheritsFrom(TGraphicControl)) then
                lua_pushobject(L, TGraphicControl(Comp).Canvas, -1)
          end else
          // TCollection Items
          if lowercase(lua_tostring(L,2)) = 'items' then begin
            lua_pushItems(L,TCollection(Comp));
          end else
          // TStrings
          if lowercase(lua_tostring(L,2)) = 'strings' then begin
            lua_pushStrings(L,TCollection(Comp));
          end else
          if lowercase(lua_tostring(L,2)) = 'cells' then begin
            lua_pushCells(L,TPersistent(Comp));
          end else
          if pos('TLUA',UpperCase(cName))>0 then begin
             lua_pushnil(L);
          end else
