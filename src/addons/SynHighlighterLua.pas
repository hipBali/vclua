...
$Id: SynHighlighterLua.pas,v 1.1.1.1 2009/01/12 02:34:32 jhansen Exp $
...
// 2014/06/10 - added nonkeyword , modified keyword handling (vclua.sf.net)

unit SynHighlighterLua;

{$I src/addons/LuaSynEdit.inc}
...
type
  ...
  public
    ...
    function GetNonKeyWords: string;
    //vclua end
  ...
  published
    ...
    property NonKeywords: TStringlist read FNonKeywords;
    property NonKeyAttri: TSynHighlighterAttributes read fNonKeyAttri write fNonKeyAttri;
    //vclua end
...
{$ENDIF}
  ...
  SYNS_LangLua = 'Lua';
  SYNS_AttrLuaMString = 'Multi-line string';
...
begin
  ...
  if FNonKeywords.IndexOf(s)>-1 then
     Result := tkNonKeyWord
  else
  //vclua end
     Result := tkIdentifier;
...
begin
  ...
  //vclua begin
  fNonKeyAttri := TSynHighLighterAttributes.Create(SYNS_AttrKey);
  fNonKeyAttri.Style := [fsBold];
  fNonKeyAttri.Foreground := clPurple;
  ...
  FKeywords.DelimitedText := GetKeyWords;

  FNonKeywords := TStringList.Create;
  FNonKeywords.Sorted := True;
  FNonKeywords.CaseSensitive:= True;
  FNonKeywords.Duplicates := dupError;
  FNonKeywords.Delimiter := ',';
  FNonKeywords.StrictDelimiter := true;
  FNonKeywords.DelimitedText := GetNonKeyWords;
  //vclua end
...
procedure TSynLuaSyn.SetLine({$IFDEF FPC}const {$ENDIF}NewValue: String; LineNumber: Integer);
begin
  fLineRef := NewValue;
...
//vclua begin
function TSynLuaSyn.GetNonKeyWords: string;
begin
  Result :=
    'add,sub,mul,div,mod,unm,eq,lt,le,index,'+
    'newindex,call,assert,collectgarbage,dofile,error,'+
    '_G,getfenv,getmetatable,ipairs,load,loadfile,loadstring,'+
    'next,pairs,pcall,print,rawequal,rawget,rawset,select,'+
    'setfenv,setmetatable,tonumber,tostring,type,unpack,xpcall,'+
    'coroutine,create,resume,running,status,wrap,yield,module,'+
    'require,package,cpath,loaded,loaders,loadlib,path,preload,'+
    'seeall,string,byte,char,dump,find,format,gmatch,gsub,len,'+
    'lower,match,rep,reverse,upper,table,concat,insert,maxn,'+
    'remove,sort,math,abs,acos,asin,atan,atan2,ceil,cos,cosh,'+
    'deg,exp,floor,fmod,frexp,huge,ldexp,log,log10,max,min,'+
    'modf,pi,pow,rad,random,randomseed,sin,sinh,sqrt,tan,tanh,'+
    'io,close,flush,input,lines,open,output,popen,read,write,'+
    'file,seek,setvbuf,os,clock,date,difftime,execute,exit,'+
    'getenv,rename,setlocale,time,tmpname,debug,gethook,getinfo,'+
    'getlocal,getregistry,getupvalue,sethook,setlocal,setupvalue,traceback';
end;
...
begin
  Result :=
    '-- This is a sample'#13#10 +
    'local varString = "This is a string"'#13#10 +
    'local varStr2 = ''This is also a string'''#13#10 +
    'local varStr3 = [[a multi-line'#13#10 +
    'string"]]'#13#10 +
    'local varNumber = 1975.064328'#13#10 +
    ''#13#10 +
    'function Test(Param1, Param2)'#13#10 +
    '    local varTemp = 0'#13#10 +
    ''#13#10 +
    '    while varTemp < 99 do'#13#10 +
    '        varTemp = varTemp + 1'#13#10 +
    '    end'#13#10 +
    'end';
end;
