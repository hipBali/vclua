...
uses
  ...
  Forms, Controls, Dialogs, runtimetypeinfocontrols,
  Lua in 'lua.pas',
...
begin
  ...
  {$IFDEF LUA53}
     lua_pushliteral (L, 'VCLua Visual Controls for LUA (5.3)');
  {$ELSE}
     {$IFDEF  LUA52}
       lua_pushliteral (L, 'VCLua Visual Controls for LUA (5.2)');
     {$ELSE}
       lua_pushliteral (L, 'VCLua Visual Controls for LUA (5.1)');
     {$ENDIF}
  {$ENDIF}
  lua_settable (L, -3);
