-- ***************************************
-- VCLua Form Editor - TreeView image editor
-- ***************************************
...
local function configureForm(loaded, node)
  ...
  if ui.TreeViewImagesEditorDialog then
    form = ui.TreeViewImagesEditorDialog
  end
  if form then
    form.Caption = "TreeView ImageLists - " .. tostring(node.name or "TreeView")
  end
  return form, ui
end
...
function M.open(ctx, node)
  ...
  if not (node.class == "TreeView") then return false end

  local loaded = formLoader.load("forms/treeview_images_editor.vclform")
  local form, ui = configureForm(loaded, node)
  if not form then error("TreeView Images editor form not found") end
  ...
  status(ctx, "TreeView ImageLists: Images=" .. tostring(node.props.Images or "(none)") ..
    ", StateImages=" .. tostring(node.props.StateImages or "(none)"))
