-- ***************************************
-- VCLua Form Editor - PageControl image editor
-- ***************************************
...
local function configureForm(loaded, node)
  ...
  if ui.PageControlImagesEditorDialog then
    form = ui.PageControlImagesEditorDialog
  end
  if form then
    form.Caption = "PageControl Images - " .. tostring(node.name or "PageControl")
  end
  return form, ui
end
...
function M.open(ctx, node)
  ...
  local registry = ctx.registry
  if not (registry and registry.isPageControl and registry.isPageControl(node.class)) then
    return false
  end

  local loaded = formLoader.load("forms/pagecontrol_images_editor.vclform")
  local form, ui = configureForm(loaded, node)
  if not form then error("PageControl Images editor form not found") end
  ...
  local refName = selectedImageListName(cbImages, lists)
  if refName and tostring(refName) ~= "" then
    node.props.Images = refName
  else
    node.props.Images = nil
  end

  if ctx.runtime then
    ctx.runtime.applyNodeProperty(node, "Images", node.props.Images)
  end

  refreshInspector(ctx, node)
  status(ctx, "PageControl Images: " .. tostring(node.props.Images or "(none)"))
